...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = await rerenderStoredAuditReport(audit_id)
    return NextResponse.json({
      success: true,
      audit_id,
      validation_repair_count: result.validation_warnings.length,
    })
  } catch (err) {
    return NextResponse.json(
      { error: err instanceof Error ? err.message : 'Failed to re-render report' },
      { status: 500 }
    )
  }
}
