...
describe('RD-05 provider limiter', () => {
  afterEach(() => { delete process.env.GEO_PROVIDER_CONCURRENCY_CLAUDE; delete process.env.GEO_PROVIDER_CONCURRENCY_OPENAI })
  ...
  it('uses conservative configurable defaults and rejects invalid values', () => {
    expect(providerConcurrency('claude')).toBe(2)
